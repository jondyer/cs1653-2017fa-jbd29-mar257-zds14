...
*   Tools and Algorithms:
    ...
    -   SHA-256 -- We chose SHA-256 because it is recommended for a variety of applications by NIST <sup id="a3">[3](#f3)</sup>, along with salt to inhibit brute-force attacks. This will be used for two purposes: validating user passwords on the GroupServer, and hashing user Tokens in order to verify their origin. Details follow.
        ...
        +   Token hashing -- The signed hash of a Token will be used to verify the integrity of Tokens that are issued to users from the GroupServer. These Tokens will first be standardized (so that two functionally equivalent Tokens are indistinguishable) and then converted to a byte array. This array will be hashed using the SHA-256 algorithm then signed with the GroupServer's public key and sent alongside the Token itself.
